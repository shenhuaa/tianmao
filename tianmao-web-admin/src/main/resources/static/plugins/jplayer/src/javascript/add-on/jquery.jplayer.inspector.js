...
(function($, undefined) {
	...
	var methods = {
		init: function(options) {
			...
			// $.each($.jPlayer.event, function(eventName,eventType) {
			// 	structure += '<div id="' + com.tianmao.config.eventId[eventType] + '" style="float:left;">' + eventName + '</div>';
			// });
...
/*			$.each($.jPlayer.event, function(eventName,eventType) {
				if($("#" + com.tianmao.config.eventId[eventType])[0] === undefined) {
					structure += '<div id="' + com.tianmao.config.eventId[eventType] + '" style="clear:left;' + eventStyle + '">' + eventName + '</div>';
				}
			});
*/
			structure +=
			...
			if(!config.visible) {
				...
				// com.tianmao.config.updateJq.click();
			}
