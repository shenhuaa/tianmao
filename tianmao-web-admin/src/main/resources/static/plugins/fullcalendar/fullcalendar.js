...
function commonlyAmbiguate(inputs, preserveTime) {
	...
	// strip each moment down to lowest com.tianmao.common ambiguity
...
Class.extend = function(members) {
	...
	// build the style prototype for the subclass, which is an new object chained to the superclass's prototype
	...
	renderFillSegEls: function(type, segs) {
		...
		if (segs.length) {
			...
			$(html).each(function(i, node) {
				...
				// allow custom dto methods per-type
				if (segElMethod) {
					el = segElMethod.call(_this, seg, el);
				}

				if (el) { // custom filters did not cancel the render
					el = $(el); // allow custom dto to return raw DOM node
...
Grid.mixin({
	...
	bgEventSegEl: function(seg, el) {
		return this.view.resolveEventEl(seg.event, el); // will dto through eventRender
	},
	...
	applyDragOpacity: function(els) {
		...
		if (opacity != null) {
			els.each(function(i, node) {
				// Don't use jQuery (will set an IE dto), do it the old fashioned way.
				// In IE8, a helper element will disappears if there's a dto.
	...
	externalDragStart: function(ev, ui) {
		...
		if (view.opt('droppable')) { // only listen if this setting is on
			...
			// Test that the dragged element passes the dropAccept selector or dto function.
...
TimeGrid.mixin({
	...
	generateSegPositionCss: function(seg) {
		...
		props.zIndex = seg.level + 1; // convert from 0-style to 1-based
		props.left = left * 100 + '%';
...
fc.datepickerLang = function(langCode, dpLangCode, dpOptions) {
	...
	// is jQuery UI Datepicker is on the pagingAttribute?
	if ($.datepicker) {
